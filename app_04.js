...
var questionsArray = [
        ...
        // Question 02:  Born at North-Pole?
        {
            promptUser: 'So tell me ' + userName + ' , was Pingu born at the North-Pole?  Please answer "Yes" or "No":',
            checkAnswer: ["XXX", "n", "y"],  // "no" is the correct answer ! therefore 'n' at index '0'
        ...
        // Question 04:  How old is Pingu?    // Numeric input, too-high too-low + limit to 5 guesses:
        {
            ...
            checkAnswer: [5],
            answerUserFalse: ["XXX", "Sorry, I didn't understand your answer. Please try again and only type a number:"],
            correctAnswersArray: [0, 0, 0],  // index has two items already so that it inly takes three more guesses to break the while loop
        },
        // Question 05:
        {
...
for (var i = 1; i < questionsArray.length; i++) {                               // i = 1 because "0" was the "What is you name?" prompt. For-loops through all the questions in questionsArray
    ...
    while(keepAsking && questionsArray[i].correctAnswersArray.length < 6) {        // < 6 is to accomodate Question 05 & 04 where correctAnswersArray acts as a counter
        ...
        var userResponse = prompt(questionsArray[i].promptUser);                // gets string from promptUser property in questionsArray
        ...
        if (userResponseArray.length < 2 && typeof userResponseArray[0] === "string" && i < 4) {      // if the userResponseArray contains only one answerUserResponse (Question 05 has multiple) and contained a "word-string" parseInt(userResponseArray[0]) will return Nan, and this will run

            userResponseSlice = userResponseArray[0].slice(0, 1);  //slices of the first character
            console.log("Slice is: " + userResponseSlice);
            ...
            } else if (userResponseSlice.toLowerCase() === questionsArray[i].checkAnswer[2]) {
                  alert(questionsArray[i].answerUserFalse[0]);
                  question_1.textContent = questionsArray[i].answerUserFalse[0];
                  console.log("Question " + i + " Wrong Answer");
                  keepAsking = false;

            } else {
                  alert(questionsArray[i].answerUserFalse[1]);
                  console.log("--- Question " + i + " User Input error ---");
                  questionsArray[i].correctAnswersArray.push(userResponseArray[0]);
            };



        // Question 04: numeric input, too-high too-low + limit to 3 guesses:
    } else if (userResponseArray.length < 2 && typeof parseInt(userResponseArray[0]) === 'number' && guess < 4) {      // if its only one entry and a number it "shuold" be for question 04
            console.log("Question " + i);

            if (userResponseArray[0] == 5) {
                alert('Well Done! You guessed right!');
                question_4.textContent = 'Well Done! You guessed right!';
                console.log('--- Guess number: ' + questionsArray[i].correctAnswersArray.length + ' --- UserResponse: ' + userResponse + ' ---');
                keepAsking = false;

            } else if (userResponseArray[0] < 5) {
                alert('No he is older than that, and he already goes to school. Try again:');
                guess += 1;           // adds one to counter...
                console.log('--- Guess number: ' + (questionsArray[i].correctAnswersArray.length - 3) + ' --- UserResponse: ' + userResponse + ' ---');
              ...
              alert("Sorry, I didn't understand your answer. Please try again and only type a number:");    // error message, does not count as a guess
        ...
        // Question 5: multiple correct answers
            ...
            while (questionsArray[i].correctAnswersArray.length < 6) {                  // if max correct answers is reached it will break
                for (var x = 0; x < userResponseArray.length; x++) {                    // x loops through userResponseArray
                    ...
                    // errors:
                        ...
                        console.log('--- Input error, " ' + userResponseArray[x] + ' " is not a string ---');
            ...
            console.log("Congratulations! You guessed " + questionsArray[i].correctAnswersArray.length + " out of 5 of Pingu's family members correctly.");
            question_5.textContent = "You guessed " + questionsArray[i].correctAnswersArray.length + " out of 5 of Pingu's family members correctly.";
            alert("Congratulations!  You guessed " + questionsArray[i].correctAnswersArray.length + " out of 5 of Pingu's family members correctly.");
